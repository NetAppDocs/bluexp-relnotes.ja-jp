...
BlueXPでは、標準モードの使用時に ID フェデレーションを管理するための、より直感的なインターフェースが提供されるようになりました。これには、より簡単なナビゲーション、フェデレーション接続を構成するためのより明確なオプション、既存のフェデレーションの可視性の向上が含まれます。
...
このリリースでは、テスト フェイルオーバー プロセス中に VM の電源をオンにした後で、VM 上でスクリプトを実行できるようになりました。これにより、スクリプトをテストおよび検証できるようになります。たとえば、NFS マウントを更新して新しいターゲット サーバーを指すようにすることで、ゲスト アプリケーションが完全にオンラインになり、回復プロセスをエンドツーエンドでテストできるようになります。

フェイルオーバープロセスの詳細については、以下を参照してください。link:https://docs.netapp.com/us-en/bluexp-disaster-recovery/use/failover.html["アプリケーションをリモートサイトにフェイルオーバーする"] 。



==== フェイルバックプロセスの改善

フェイルオーバー中は、検出が完了するまでフェイルバック プロセスが無効になります。これにより、フェイルバックを開始する前にすべての構成が正しく識別され、考慮されるようになり、エラーのリスクが軽減されます。

フェイルバックプロセスの詳細については、以下を参照してください。link:https://docs.netapp.com/us-en/bluexp-disaster-recovery/use/failback.html["アプリケーションを元のソースにフェイルバックする"] 。
