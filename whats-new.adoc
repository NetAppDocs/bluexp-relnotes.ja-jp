...
デフォルトの監視リストまたはダッシュボードの[Upgrade Advisor]ページには、 link:https://activeiq.netapp.com/redirect/upgrade-advisor["Upgrade Advisor を使用します"^] リンクデフォルトの監視リストやダッシュボードがない場合は、リンクをクリックすると検索ページにリダイレクトされます。



==== アップグレードプラン

アップグレードプランは、冗長なアップグレード手順を削除し、情報と警告を変更し、バックアウトプランを簡素化するように最適化されています。クラスタ内のすべてのノードで共通の手順が統合され、[General Information]セクションで使用できます。改善された計画を表示するには、に記載されている手順に従ってアップグレード計画を生成します。 link:https://docs.netapp.com/us-en/active-iq/task_view_upgrade.html["アップグレードプランを使用したONTAPのバージョンのアップグレード"]。
